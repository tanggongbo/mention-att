...
def load_pretrained_component_from_model(
    ...
    for key in state["model"].keys():
        if key.startswith(component_type):
            ...
            if component_state_dict[component_subkey].shape != state["model"][key].shape:
                print(component_state_dict[component_subkey].shape, state["model"][key].shape)
                component_state_dict[component_subkey][:state["model"][key].shape[0]] = state["model"][key]
            else:
