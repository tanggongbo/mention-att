...
class SequenceGenerator(nn.Module):
    ...
    @torch.no_grad()
    # changes compared to conventional code
    def _source_ne(self, encoder_outs, model):
        return [m.encoder_ne_process(encoder_outs[i], True)[1] for (i, m) in enumerate(model.models)]
    ...
    def _generate(
        ...
        # changes: get the src-side mention prediction
        # encoder_ne_logit = self._source_ne(encoder_outs, self.model)[0]
        # mention_tag_src = encoder_ne_logit.argmax(dim=-1)  # BxTxC->BxT
        # for idx in range(mention_tag_src.shape[0]):
        #     print(f'mention_tag_src: {mention_tag_src[idx]}')
...
class EnsembleModel(nn.Module):
    ...
    def forward_decoder(
        ...
        # changes: instead of calling forward(), call _forward_decoder(),
        # consider two decoders instead of the base decoder
        # TODO: UPDATE incremental_states, two? for both decoders?
        for i, model in enumerate(self.models):
            if self.has_encoder():
                encoder_out = encoder_outs[i]
            # decode each model
            if self.has_incremental_states():
                decoder_out = model._forward_decoder(tokens, encoder_out=encoder_out,
                                                     incremental_state=incremental_states[i],)
            else:
                # decoder_out = model.decoder.forward(tokens, encoder_out=encoder_out)
                decoder_out = model._forward_decoder(tokens, encoder_out=encoder_out)

            # decoder_ne_logit = decoder_out.decoder_ne_logit
            # mention_tag_trg = decoder_ne_logit.argmax(dim=-1)
            # for idx in range(mention_tag_trg.shape[0]):
            #     print(f'mention_tag_trg: {mention_tag_trg[idx]}')
    ...
    def reorder_incremental_state(
        ...
        for i, model in enumerate(self.models):
            ...
            model.decoder4mention.reorder_incremental_state_scripting(
                incremental_states[i], new_order
            )
